...
#
# DESCRIPTION:
#    marsutil(n): Order Flunky Class
#
#    order_flunky(n) is a class that manages the execution of order orders,
...
oo::class create ::marsutil::order_flunky {
    #-------------------------------------------------------------------
    # Instance Variables
    ...
    method execute {mode order} {
        ...
        ::marsutil::require {$mode in {gui normal private}} "Invalid mode: \"$mode\""
        ::marsutil::require {[my available [$order name]]} \
            "Order [$order name] is not available in state \"[my state]\""
        ::marsutil::require {[$order valid]} "This [$order name] order is invalid."
    ...
    method InTransaction {} {
        return [::kiteutils::got $transList]
    }
    ...
    method canundo {} {
        ...
        return [::kiteutils::got $undoStack]
    }
    ...
    method canredo {} {
        ...
        return [::kiteutils::got $redoStack]
    }
    ...
    method undo {} {
        ::marsutil::require {![my InTransaction]} "Cannot undo during transaction."
    ...
    method redo {} {
        ::marsutil::require {![my InTransaction]} "Cannot redo during transaction."
    ...
    method UndoPushNew {item} {
        ...
        if {[my InTransaction]} {
            if {[$order canundo]} {
                ::kiteutils::lpush transList $order
            } else {
                error "Non-undoable order used in transaction: [$order name]"
            }
        } else {
            if {[$order canundo]} {
                ::kiteutils::lpush undoStack $order
            } else {
                my UndoClear
            }
        }
